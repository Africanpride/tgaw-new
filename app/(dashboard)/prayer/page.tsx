...
export default async function PrayerPage(props: {
	...
	const accent = slotAccent["PRAYER"];
	...
	return (
		<div className="flex flex-col gap-6">
			...
			<Card className="min-w-0 w-full overflow-hidden border-red-500/20 shadow-md">
				<CardContent className="flex items-center justify-between p-6">
					<DateNav dateStr={dateStr} basePath="/prayer" />
				</CardContent>
				<CardContent className="pt-0">
					<SlotBookingStrip slots={slots} type="PRAYER" initialSlotId={initialSlotId} />
				</CardContent>
			</Card>
					...
					<CardContent>
						{myBookings.length === 0 && (displacedBookings?.length ?? 0) === 0 ? (
							<p className="text-sm text-muted-foreground">
								No prayer sessions scheduled for this day.
							</p>
						) : (
							<div className="space-y-3">
								<DisplacedBookingNotice bookings={displacedBookings ?? []} slotNoun="prayer" />
