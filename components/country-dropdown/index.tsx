...
const CountryDropdownComponent = (
  ...
  const [manualSelection, setManualSelection] = useState<Country[]>([]);

  const selectedCountries = useMemo<Country[]>(() => {
    if (manualSelection.length > 0) return manualSelection;

    // Resolve defaultValue (alpha3 codes) against the available options.
    if (multiple && Array.isArray(defaultValue)) {
      return options.filter((country) =>
        (defaultValue as string[]).includes(country.alpha3)
      );
    }
    if (!multiple && typeof defaultValue === "string" && defaultValue) {
      const found = options.find(
        (country) => country.alpha3 === (defaultValue as string)
      );
      return found ? [found] : [];
    }
    return [];
  }, [options, defaultValue, multiple, manualSelection]);
  ...
  const triggerClasses = cn(
    "flex h-9 w-full items-center justify-between whitespace-nowrap rounded-md border border-input bg-transparent px-3 py-2 text-sm shadow-xs ring-offset-background placeholder:text-muted-foreground focus:outline-none disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1 hover:bg-secondary/80",
